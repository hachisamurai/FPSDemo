...
购买权限、确认报价校验、金币扣费和解锁保存/回滚已集中到`UDemoShopComponent`；`UDemoAmmoComponent.Purchase/GetBlockReason`保留为兼容转发。弹药库存、装配GE、Capture/Restore仍属于Pawn弹药组件，UI调用与存档格式不变。详细调用链和本轮`SystemsAmmo.log`验证见[系统组件与职责划分](系统组件与职责划分.md)。
