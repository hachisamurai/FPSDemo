...
void ADemoEnemy::Configure(const FDemoEnemySpawnStats& Stats)
{
	...
	// 动态材质实例归网格持有，单个敌人的颜色变化不污染共享材质。
	if (UMaterialInstanceDynamic* Material = Visual->CreateAndSetMaterialInstanceDynamic(0)) // 网格拥有独立MID，用颜色辨认角色。
	{
		const FLinearColor RoleColor = Tactics->GetRole()==EDemoEnemyRole::Ranged ? FLinearColor(.04f,.35f,.9f)
			: (Tactics->GetRole()==EDemoEnemyRole::Flanker ? FLinearColor(.6f,.08f,.8f) : FLinearColor(.8f,.04f,.12f)); // 远程蓝、侧翼紫、追击红。
...
void ADemoEnemy::OnHealthChanged(const FOnAttributeChangeData& Data)
{
	...
	if (AFPSDemoGameMode* Mode = GetWorld()->GetAuthGameMode<AFPSDemoGameMode>()) Mode->NotifyEnemyKilled(this);
...
void ADemoEnemy::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (!bDead && EndPlayReason == EEndPlayReason::Destroyed)
		if (AFPSDemoGameMode* Mode = GetWorld()->GetAuthGameMode<AFPSDemoGameMode>()) Mode->NotifyEnemyLost(this);
