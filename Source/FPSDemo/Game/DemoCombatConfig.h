...
/** 难度枚举稳定映射 Easy/Normal/Hard 行名；不以 UI 显示文本作为配置键。 */
UENUM(BlueprintType)
enum class EDemoDifficulty : uint8 { Easy, Normal, Hard };
...
/** 全局难度倍率表，三个固定行名 Easy、Normal、Hard。 */
USTRUCT(BlueprintType)
...
struct FPSDEMO_API FDemoLevelRow : public FTableRowBase
{
	...
	// 可选 Boss 模板行名；None 表示没有 Boss，最多一只。
...
struct FPSDEMO_API FDemoEnemySpawnStats
{
	...
	int32 FormationSlot = 0; // GM在逐怪生成时填写的非负序号，用于混编与左右分工。
