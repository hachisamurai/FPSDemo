...
EDemoQuitState ADemoPlayerController::GetQuitState() const { DEMO_LOG_TICK(); return QuitState; }
bool ADemoPlayerController::IsQuitLocalSaved() const { DEMO_LOG_TICK(); return bQuitLocalSaved; }
...
void ADemoPlayerController::QuitPressed()
{
    ...
    if (!GetWorld()->IsPaused() && !SetPause(true)) { MenuMessage = TEXT("暂停失败，暂时无法安全退出"); UE_LOG(LogFPSDemo, Warning, TEXT("EXIT_SAVE pause failed")); return; }
...
bool ADemoPlayerController::SaveBeforeQuit()
{
    DEMO_LOG_CALL();
    UDemoPlayerProfile* Profile = GetGameInstance()->GetSubsystem<UDemoPlayerProfile>(); // GI持有永久档案，退出前显式写盘，不依赖Deinitialize。
    ...
    else if (!Mode->SaveCheckpoint()) MenuMessage = GetGameInstance()->GetSubsystem<UDemoRunSaves>()->GetStatus();
...
void ADemoPlayerController::UpdateQuitSave()
{
    DEMO_LOG_TICK();
    ...
    if (QuitState == EDemoQuitState::SavingLocal && FPlatformTime::Seconds() >= QuitNextActionTime)
    {
        ...
        UDemoCloudSync* Cloud = GetGameInstance()->GetSubsystem<UDemoCloudSync>(); // Editor永远为空，不创建HTTP或设置联网豁免。
    ...
    {
        UDemoCloudSync* Cloud = GetGameInstance()->GetSubsystem<UDemoCloudSync>(); // GI比控制器长寿；仅此页轮询，回调从不捕获控制器。
...
void ADemoPlayerController::QuitWithLocalSave()
{
    ...
    if (UDemoCloudSync* Cloud = GetGameInstance()->GetSubsystem<UDemoCloudSync>()) Cloud->CancelExitSync(); // 保留可能已提交但未收到回执的同一请求。
    if (!SaveBeforeQuit()) return;
    QuitState = EDemoQuitState::Saved;
    QuitNextActionTime = FPlatformTime::Seconds() + 1.0;
...
void ADemoPlayerController::CancelQuitSave()
{
    ...
    if (UDemoCloudSync* Cloud = GetGameInstance()->GetSubsystem<UDemoCloudSync>()) Cloud->CancelExitSync(); // 弱UObject HTTP仍由GI管理，不依赖页面生命周期。
    QuitState = EDemoQuitState::Idle;
    MenuPage = QuitBackPage;
    bPauseMenuActive = MenuPage == EDemoMenuPage::Pause;
    if (!bPauseMenuActive) SetPause(false);
    MenuMessage = TEXT("已取消退出");
    RefreshMenuInput();
    UE_LOG(LogFPSDemo, Log, TEXT("EXIT_SAVE cancelled"));
}
void ADemoPlayerController::FinishQuit()
{
    DEMO_LOG_CALL();
